...
export default defineConfig(({ mode }) => {
  const common = {
    ...
    server: {
      ...
      proxy: {
        '/outputs': {
          target: 'http://35.209.222.13:9090/',
          changeOrigin: true,
          // rewrite: (path) => path.replace(/^\/outputs/, ''),
        },
        '/upload': {
          target: 'http://35.209.222.13:9090/upload',
          changeOrigin: true,
          // rewrite: (path) => path.replace(/^\/upload/, ''),
        },
        '/flaskwebgui-keep-server-alive': {
          target: 'http://35.209.222.13:9090/flaskwebgui-keep-server-alive',
          changeOrigin: true,
          // rewrite: (path) =>
